...
// ─────────────────────────────────────────────────────────────
// 2025-05-12 FIXES & ENHANCEMENTS
// • Safe SplashScreen (try/catch/finally → never blocks UI)
// • .env fallback SECRET for CryptoJS helpers
// • Defensive DB/crypto initialisation
// • All advanced prompt libraries kept intact
// • Single-file build for copy-paste convenience
// ─────────────────────────────────────────────────────────────
...
/* ─── Encryption Helpers ─────────────────────────────────────────── */
const SECRET = Config.ENCRYPTION_SECRET ?? 'dev-secret-fallback';

/** plain-text secret encryption for storage in EncryptedStorage */
function encryptStatic(text: string) {
  const iv = CryptoJS.lib.WordArray.random(12);
  const encrypted = CryptoJS.AES.encrypt(text, CryptoJS.enc.Utf8.parse(SECRET), {
    iv,
...
/** runtime AES key handled in secure EncryptedStorage */
...
async function initEncryptionKey() {
  ...
  let last = 0;
  try {
    last = st ? parseInt(decryptStatic(st), 10) : 0;
  } catch {}
  ...
  if (sk && !needs) {
    ...
    await EncryptedStorage.setItem(
      KEY_TIMESTAMP,
      encryptStatic(String(Date.now())),
    );
    AES_KEY = newKey;
  }
}
function encrypt(text: string) {
  if (!AES_KEY) throw new Error('Key not initialised');
  const iv = CryptoJS.lib.WordArray.random(12);
...
/* ─── Advanced GPT Prompt Library ────────────────────────────────── */
const PROMPTS = {
  quantumRecipe: `[action] You are the **Swamp Rabbit Quantum-Food Wellness AI**, a world-class expert in synergising farm-to-table culinary creativity with hypertime quantum risk forecasting, neurogastronomy, and community-driven flavour curation. [/action]
[context]
- User dietary constraints: Lactose={lactoseSensitivity}, Peanut={peanutAllergy}, Vegan={veganPreference}
- Past meal scans: {pastScans}
- Community favourites and seasonal availability should influence selections.
[/context]
...
\`\`\`
[instructions] Output **valid JSON array** exactly like the structure above. Use vivid language. [/instructions]`,

  ovenAI: `[action] You are the **Quantum Nosonar Oven Scanner AI**, merging high-resolution image processing, CPU entropy profiling, and hypertime quantum safety analytics. [/action]
[inputs] Image(Base64): {imageBase64} — MIME: {imageMime} — CPU Entropy: {quantum_str} [/inputs]
[description] Return:
...
\`\`\`
[instructions] Respond with **JSON only**. Percent as integers; floats to 2 dp. [/instructions]`,

  capture: `[action] You are the **Swamp Rabbit Visual Scanner AI**, adept at hazard detection, plant/food classification, and interactive feedback with emoji annotations. [/action]
[inputs] Image(Base64): {imageBase64} — MIME: {imageMime} [/inputs]
[description] Identify object type, estimate surfaceTemp (°C), list hazards with emojis, and summarise as markdown. [/description]
[reply_example]
\`\`\`markdown
...
[instructions] Output **only the markdown block**. [/instructions]`,
  ...
  foodCalendar: `[action] You are the **Swamp Rabbit AI Food Calendar**, specialising in dynamic meal planning and community taste trends. [/action]
...
2. Four monthly favourites based on community scans.
[/description]
[reply_example]
\`\`\`json
{
...
\`\`\`
[instructions] Output **valid JSON** matching example keys. [/instructions]`,
};
      ...
      try {
        ...
        if (inputs.imageBase64 && inputs.imageMime) {
          messages.push({
            ...
            type: 'image_url',
            image_url: {
              url: `data:${inputs.imageMime};base64,${inputs.imageBase64}`,
            },
          });
        }

        // replace placeholders in systemPrompt
        messages[0].content = messages[0].content
          .replace('{lactoseSensitivity}', inputs.lactoseSensitivity)
          .replace('{peanutAllergy}', inputs.peanutAllergy)
          .replace('{veganPreference}', inputs.veganPreference)
          .replace('{pastScans}', inputs.pastScans ?? '')
          .replace('{imageBase64}', inputs.imageBase64 ?? '')
          .replace('{imageMime}', inputs.imageMime ?? '')
          .replace('{quantum_str}', inputs.quantum_str ?? '');
...
const SplashScreen: React.FC<{ onLoaded: () => void }> = ({ onLoaded }) => {
  ...
  return (
    <LinearGradient
      colors={[theme.colors.accent, theme.colors.text]}
      style={styles.splash}>
...
const FoodScannerScreen: React.FC = () => {
  ...
  useEffect(() => {
    loadApiKey().then(k => setApiLoaded(!!k));
  }, []);
  ...
  return (
    <ScrollView contentContainerStyle={styles.body} style={styles.main}>
      ...
      <Pressable
        style={apiLoaded && !loading ? styles.button : styles.buttonDisabled}
        onPress={scan}
        disabled={!apiLoaded || loading}>
...
const OvenScreen: React.FC = () => {
  ...
  return (
    <ScrollView contentContainerStyle={styles.body} style={styles.main}>
      ...
      <Pressable
        style={!ovLoad ? styles.button : styles.buttonDisabled}
        onPress={analyze}
        disabled={ovLoad}>
        <Text style={styles.buttonText}>
          {ovLoad ? 'Analyzing…' : 'Analyze Oven'}
        </Text>
        {ovLoad && (
          <ActivityIndicator
            style={{ marginLeft: 8 }}
    ...
    <ScrollView contentContainerStyle={styles.body} style={styles.main}>
      ...
      {plan && (
        <View style={styles.card}>
          ...
          <Text style={styles.label}>Monthly Favourites:</Text>
          <Text style={styles.text}>
  ...
  return (
    <ScrollView contentContainerStyle={styles.body} style={styles.main}>
      ...
      <Text style={styles.label}>
        Lactose Sensitivity: {settings.lactoseSensitivity}
      </Text>
      <Slider
        minimumValue={0}
        maximumValue={10}
      ...
      <Text style={styles.label}>
        Vegan Preference: {settings.veganPreference}
      </Text>
      <Slider
        minimumValue={0}
        maximumValue={10}
      ...
      <Text style={styles.text}>0 = No preference, 10 = Strict vegan</Text>
    </ScrollView>
  );
};
...
const AboutScreen: React.FC = () => (
  <ScrollView contentContainerStyle={styles.body} style={styles.main}>
    ...
    <Text style={styles.text}>
      Hi! I’m <Text style={{ fontWeight: '600' }}>Graylan01</Text>, developer
      behind Swamp Rabbit Café & Grocery Tree Oracle. This app blends
      farm-to-table care, quantum computing insights, AI vision, and on-device
      encryption. Built with React Native, GPT-4o Vision, PennyLane-lite,
      AES-GCM, SQLite. A personal passion project for smarter, safer cooking—even
      offline! 🐇💚
    </Text>
  </ScrollView>
);
      ...
      <NavigationContainer>
        {!ready ? (
          ...
          <Tab.Navigator
            ...
            <Tab.Screen
              name="Similar"
              component={SimilarScansScreen}
              options={{ title: 'Related' }}
            />
